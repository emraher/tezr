...
#'
#' @name detail
#' @param detail_id Character vector. The encoded thesis ID(s) from the
#'   `detail_id` column of search results. Required. Accepts multiple values
#'   for batch retrieval.
...
fetch_batch_details <- function(detail_id, progress = TRUE) {
  ...
  split <- split_cached_uncached(valid_ids)
...
fetch_uncached_parallel <- function(uncached_ids, results, progress = FALSE) {
  ...
  for (chunk_start in chunk_starts) {
    ...
    if (isTRUE(progress)) {
      cli::cli_alert_info(
        "Fetched {.val {completed}}/{.val {total_uncached}} uncached detail record{?s}"
      )
    }
  }
...
fetch_single_thesis <- function(detail_id) {
  ...
  req <- create_session(apply_rate_limit = detail_rate_limit_enabled()) |>
    ...
    httr2::req_url_query(id = detail_id) |>
...
build_detail_request <- function(detail_id) {
  create_session(apply_rate_limit = detail_rate_limit_enabled()) |>
    httr2::req_url(paste0(base_url, endpoints$detail)) |>
    httr2::req_url_query(id = detail_id) |>
    httr2::req_retry(max_tries = 3, backoff = ~2) |>
    httr2::req_error(is_error = function(resp) FALSE)
...
parse_and_cache_detail_response <- function(
  ...
  details$detail_url <- paste0(base_url, endpoints$detail, "?id=", detail_id)
